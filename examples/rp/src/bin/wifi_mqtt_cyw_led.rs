...
async fn main(spawner: Spawner) {
    info!("Pico W MQTT (CYW LED) booting...");
    ...
    'reconnect: loop {
        ...
        match mqtt_expect_connack(&mut sock).await {
            Ok(()) => {
                ...
                for _ in 0..2 {
                    let _ = control.gpio_set(0, true).await;   // 若極性相反，改成 false
...
fn build_json_payload() -> heapless::String<256> {
    ...
    let heater_coil_ma = 182.0; // 暖風機交流線圈電流 (mA)
    let temp_c = 26.5;          // 溫度
    let rh = 64.0;              // 相對溼度
