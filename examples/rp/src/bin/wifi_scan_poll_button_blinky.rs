//! Pico W: 按下 GPIO15 觸發 Wi-Fi 掃描（每按一次就掃一次）+ GPIO22 LED + 掃描期間 Performance
...
async fn main(spawner: Spawner) {
    ...
    // LED：GPIO22，預設熄滅（Low）。掃描中點亮（High）
    ...
    loop {
        ...
        {
            // 建立 scanner（注意：scanner 存在期間會借用 control）
            let mut scanner = control.scan(Default::default()).await;

            while let Some(bss) = scanner.next().await {
                // SSID 可能為空（隱藏）或非 UTF-8
        ...
        } // scanner 在此 drop，釋放對 control 的借用

        // 掃描結束：切回 PowerSave、滅 LED
        control
            .set_power_management(cyw43::PowerManagementMode::PowerSave)
            .await;
        led.set_low();
